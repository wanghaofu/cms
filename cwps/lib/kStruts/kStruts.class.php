...
class kStruts
{
	...
	function kStruts( &$STRUTS_CONFIG, &$IN )
	{
		global $db_config;
		global $db;
		global $SYS_ENV;
		...
		if ( !isset( $action_cfg['tpl'] ) || $action_cfg['tpl'] )
		{
			...
			$this->TPL->template_dir = ROOT_PATH.$STRUTS_CONFIG['template-resources'].DS;
			$this->TPL->compile_dir = TMP_DIR."templates_c".DS;
			$this->TPL->cache_dir = TMP_DIR."cache".DS;
			$this->TPL->compile_lang = true;
			$this->TPL->global_lang_name = ROOT_PATH.$STRUTS_CONFIG['message-resources']['language-dir'].DS.$SYS_ENV['language'].DS.$STRUTS_CONFIG['message-resources']['templates-global-messages-resource'];
			$this->TPL->lang_dir = ROOT_PATH.$STRUTS_CONFIG['message-resources']['language-dir'].DS.$SYS_ENV['language'].DS.$STRUTS_CONFIG['message-resources']['template-messages-package'].DS;
...
//		if ( !empty( $this->config['before-filter'] ) )
//		{
//			foreach ( $this->config['before-filter'] as $class_string => $var )
//			{
//				$class_path = str_replace( ".", "/", $class_string );
//				$class_name = str_replace( ".", "_", $class_string );
//				include_once( CLS_PATH.$class_path.".php" );
//				de(CLS_PATH.$class_path.".php",__file__,__line__ ,0);
//				de($class_name,__file__,__line__);
//				$Filter = new $class_name( );
//				$Filter->doFilter( $ActionMapping, $IN, $this->TPL );
//				unset( $Filter );
//			}
//		}
	...
	{
		if ( substr( $actionNS, -6 ) == "Action" )
		{
			$className = array_pop( explode( ".", $actionNS ) );
		}
		else
		{
			$className = str_replace( ".", "_", $actionNS );
		}
		return $className;
	}
...
class kStruts_AfterFilter
{

	var $params = array( );

	function doFilter( &$IN, &$TPL )
	{
	}
...
class ActionMapping
{
	...
	function doForwardAction( $url_forward, $jumpMsg = "", $jumpTime = 1, $params = array( ) )
	{
		...
		if ( preg_match( "/^[0-9A-Za-z]+\\.[0-9A-Za-z]+/isU", $jumpMsg ) )
		{
			...
			else if ( !empty( $params ) )
			{
				foreach ( $params as $key => $var )
				{
					...
					if ( empty( $key ) )
					{
						$evalStr = "sprintf(\$_LANG_MESSAGES[\$jumpMsg], \"{$var}\"";
					}
					else
					{
						$evalStr .= ", \"{$var}\"";
		...
		if ( $STRUTS_CONFIG['session-id-store'] == "page" )
		{
			...
			{
				$this->TPL->assign( "url_forward", "{$STRUTS_CONFIG['entrance']}?{$session_id_name}={$this->sId}&amp;do=".$url_forward );
			}
		...
		{
			$this->TPL->assign( "url_forward", "{$STRUTS_CONFIG['entrance']}?do=".$url_forward );
		}
		else if ( empty( $url_forward ) )
		{
			$this->TPL->assign( "url_forward", $STRUTS_CONFIG['entrance'] );
	...
	function doForward( $tplname )
	{
		...
		if ( !empty( $this->strutsConfig['after-filter'] ) )
		{
			foreach ( $this->strutsConfig['after-filter'] as $class_string => $var )
			{
				$class_path = str_replace( ".", "/", $class_string );
				$class_name = str_replace( ".", "_", $class_string );
				include_once( CLS_PATH.$class_path.".php" );
				$Filter = new $class_name( );
				$Filter->doFilter( $IN, $this->TPL, $tplname );
				unset( $Filter );
			}
		}
...
function getActionURL( $_actionName )
{
	global $STRUTS_CONFIG;
	global $IN;
	$session_id_name = $STRUTS_CONFIG['session-id-name'];
	$base_url = $STRUTS_CONFIG['entrance']."?{$session_id_name}=".$IN[$session_id_name]."&amp;"."do=".$_actionName."&amp;";
	return $base_url;
}
