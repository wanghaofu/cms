...
class kTemplate_Compiler extends kTemplate
{
	...
	(
		0 => "parse_literal_encode",
		1 => "parse_html_tag_input_radio",
		2 => "parse_html_tag_select",
		3 => "parse_xml",
		4 => "parse_php",
		5 => "parse_if_cmsware3",
		6 => "parse_if",
	...
	function compile( $file_name, $compile_name = NULL )
	{
		...
		else if ( File::autowrite( $this->compile_dir.$this->compilefile_prefix.$data['basename'], $content ) )
		{
	...
	function parse_include_format( $string )
	{
		...
		if ( strpos( "hawking".$string, "file:" ) )
		{
			...
			{
				$string = "<b>kTemplate error : </b>unable to compile template: <b>{$string}</b>";
			}
		}
		...
		else if ( strpos( "hawking".$string, "../" ) )
		{
			...
			$new_name = $this->compilefile_prefix.$this->format( $this->template_dir.$pathinfo['dirname'].$string );
			...
			foreach ( $matches[1] as $key => $var )
			{
				...
				if ( preg_match_all( "/<option value=\"(.*)\"[^>]*>(.*)<\\/option>/isU", $matches[2][$key], $optionMatches ) )
				{
					foreach ( $optionMatches[0] as $keyIn => $varIn )
					{
						...
						$replace .= "<input type=\"radio\" name=\"{$attributes['name']}\" value=\"{$value}\" id=\"{$attributes['name']}{$keyIn}\" <?php echo \${$attributes['name']}{$value} ;?>><label for=\"{$attributes['name']}{$keyIn}\"  >{$label}</label>&nbsp;";
					}
				}
				$contents = str_replace( $matches[0][$key], $replace, $contents );
			}
