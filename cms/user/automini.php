...
class AutoMini 
{
	...
	{	 
		if(isset($IN['copyright'])) {
			...
			die("<H1>AutoMini ".CLASS_VERSION."</H1> <HR>Copyright &copy; 1999-".date("Y")." <A HREF='http://www.localhost.com'>CMSware</A>&trade;. All rights reserved.<BR>CMSware缁跷欢锛屽晢涓氲蒋浠讹纴链粡鍏佽锛屼笉寰楁搮镊娇鐢ㄥ拰鎾掓挲锛岃缭钥呭繀绌讹紒 ");
		}
		...
		if($this->cache) {
			$this->cacheFile = PHOTO_CACHE_PATH."cache.automini.".md5($this->src.$this->miniMode).'.'.str_replace('*','x',$this->pixel).'.'.$this->miniType;
		}
 	}
