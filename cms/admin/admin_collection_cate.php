...
case "edit_submit" :
	...
	if ( $collection_cate->update( $IN[CateID] ) )
	{
		foreach ( $contentRules as $key => $var )
		{
...
case "del" :
	...
	else if ( $collection_cate->haveSon( $IN[CateID] ) && $IN[action] == "force" )
	{
	...
	else if ( $collection_cate->del( $IN[CateID] ) )
	{
...
case "move" :
	...
	else if ( $collection_cate->update( $IN[CateID] ) )
	{
...
case "import_submit" :
	...
	if ( !empty( $content ) )
	{
		...
		if ( $collection_cate->update( $CateInfo[CateID] ) )
		{
