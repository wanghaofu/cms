...
switch($IN[o]) {
		...
		case 'edit':
			require_once INCLUDE_PATH."admin/TplVarsAdmin.class.php";
			if(empty($IN[IndexID])) _goto('content_list');
		...
		case 'topIt_submit':
			if(!empty($IN[pData])) {
				...
				if($result) showmessage('top_multi_ok', $referer);
				else showmessage('top_multi_fail', $referer);
				...
				showmessage('top_multi_fail', $referer);
			}
			break;
		case 'pinkIt':
			if(!empty($IN[IndexID])) {
		...
		case 'pinkIt_submit':
			if(!empty($IN[pData])) {
				...
				if($result) showmessage('pink_multi_ok', $referer);
				else showmessage('pink_multi_fail', $referer);
				...
				showmessage('pink_multi_fail', $referer);
