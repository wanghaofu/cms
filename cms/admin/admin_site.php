...
function getPagersList( )
{
	...
	while ( $file = $dir->read( ) )
	{
		...
		else if ( is_dir( SYS_PATH."setting/pager/".$file ) )
		{
			continue;
		}
		else
...
function getEditorsList( )
{
	...
	while ( $file = $dir->read( ) )
	{
		...
		else if ( is_dir( SYS_PATH.ADMIN_NAME."/editor/".$file ) )
		{
			continue;
		}
		else
...
case "add" :
	...
	if ( empty( $IN[ParentID] ) )
	{
		...
		if ( !$site->isSiteAdmin( ) )
		{
			goback( "site_permission_deny_notadmin" );
		}
	}
	else
	{
	...
	foreach ( $gInfo as $var )
	{
		if ( in_array( $var['gId'], $PermissionWriteG ) )
		{
			$TargetPermissionWriteG[] = $var;
		}
		else
		{
			$SrcPermissionWriteG[] = $var;
		}
	}
	foreach ( $gInfo as $var )
	{
		if ( in_array( $var['gId'], $PermissionManageG ) )
		{
			$TargetPermissionManageG[] = $var;
		}
		else
		{
			$SrcPermissionManageG[] = $var;
		}
	}
	foreach ( $gInfo as $var )
	{
		if ( in_array( $var['gId'], $PermissionApproveG ) )
		{
			$TargetPermissionApproveG[] = $var;
		}
		else
		{
			$SrcPermissionApproveG[] = $var;
		}
	}
	foreach ( $gInfo as $var )
	{
		if ( in_array( $var['gId'], $PermissionPublishG ) )
		{
			$TargetPermissionPublishG[] = $var;
		}
		else
		{
			$SrcPermissionPublishG[] = $var;
		}
	}
	foreach ( $uInfo as $var )
	{
		if ( in_array( $var['uId'], $PermissionReadU ) )
		{
			$TargetPermissionReadU[] = $var;
		}
		else
		{
			$SrcPermissionReadU[] = $var;
		}
	}
	foreach ( $uInfo as $var )
	{
		if ( in_array( $var['uId'], $PermissionWriteU ) )
		{
			$TargetPermissionWriteU[] = $var;
		}
		else
		{
			$SrcPermissionWriteU[] = $var;
		}
	}
	foreach ( $uInfo as $var )
	{
		if ( in_array( $var['uId'], $PermissionManageU ) )
		{
			$TargetPermissionManageU[] = $var;
		}
		else
		{
			$SrcPermissionManageU[] = $var;
		}
	}
	foreach ( $uInfo as $var )
	{
		if ( in_array( $var['uId'], $PermissionApproveU ) )
		{
			$TargetPermissionApproveU[] = $var;
		}
		else
		{
			$SrcPermissionApproveU[] = $var;
		}
	}
	foreach ( $uInfo as $var )
	{
		if ( in_array( $var['uId'], $PermissionPublishU ) )
		{
			$TargetPermissionPublishU[] = $var;
		}
		else
		{
			$SrcPermissionPublishU[] = $var;
		}
	}
...
case "edit_submit" :
	...
	if ( $site->update( $IN[NodeID] ) )
	{
		...
		{
			showmessage( "edit_site_ok", $referer );
		}
	}
	else
	{
...
case "del" :
	...
	else if ( $site->haveSon( $IN[NodeID] ) && $IN[action] == "force" )
	{
