...
case "addtable_submit" :
	...
	if ( !empty( $IN['tName'] ) )
	{
		...
		if ( $content_table->addTable( $IN['basedTableID'] ) )
		{
			...
			_goto( "edittable&TableID={$content_table->db_insert_id}" );
		}
	}
	else
	{
		_goto( "viewtable", "addtable_name_null" );
	}
	break;
case "edittable" :
	if ( !empty( $IN['TableID'] ) )
...
case "edittable_submit" :
	if ( $IN['TableID'] != "" )
	{
		...
		if ( $content_table->updateTable( $IN['TableID'] ) )
		{
			...
			$cache->makeCache( "content_model" );
			cleardir( SYS_PATH."sysdata/cache/", "index.html;.htaccess" );
			_goto( "edittable&TableID={$IN['TableID']}", "edittable_submit_ok" );
		}
		else
		{
			_goto( "edittable&TableID={$IN['TableID']}", "edittable_submit_fail" );
		}
	}
	else
	{
...
case "deltable" :
	if ( $IN['TableID'] != "" )
	{
		if ( $content_table->delTable( $IN['TableID'] ) )
		{
			...
			$cache->makeCache( "content_model" );
			cleardir( SYS_PATH."sysdata/cache/", "index.html;.htaccess" );
			_goto( "viewtable", "deltable_ok" );
		}
		else
		{
			_goto( "viewtable", "deltable_fail" );
		}
	}
	break;
case "addfield" :
...
case "addfield_submit" :
	...
	if ( $IN['data']['FieldName'] != "" && $IN['data']['FieldTitle'] != "" && $IN['data']['FieldType'] != "" && $IN['TableID'] != "" )
	{
		...
		if ( $content_table->addField( $IN['TableID'], $IN['data'] ) )
		{
			...
			$cache->makeCache( "content_model" );
			cleardir( SYS_PATH."sysdata/cache/", "index.html;.htaccess" );
			_goto( "edittable&TableID={$IN['TableID']}", "table_add_ok" );
		}
		else
		{
			_goto( "edittable&TableID={$IN['TableID']}", "table_add_fail" );
		}
	}
	else
	{
		_goto( "addfield&TableID={$IN['TableID']}", "table_add_data_null" );
	}
	break;
case "editfield" :
...
case "editfield_submit" :
	if ( $IN['data']['FieldName'] != "" && $IN['data']['FieldTitle'] != "" && $IN['data']['FieldType'] != "" && $IN['TableID'] != "" && $IN['ContentFieldID'] != "" )
	{
		...
		if ( $content_table->editField( $IN['ContentFieldID'], $IN['data'] ) )
		{
			...
			$cache->makeCache( "content_model" );
			cleardir( SYS_PATH."sysdata/cache/", "index.html;.htaccess" );
			_goto( "edittable&TableID={$IN['TableID']}", "table_editfield_ok" );
		}
		else
		{
			_goto( "edittable&TableID={$IN['TableID']}", "table_editfield_fail" );
		}
	}
	else
	{
		_goto( "editfield&TableID={$IN['TableID']}", "table_editfield_data_null" );
	}
	break;
case "delfield" :
	if ( $IN['ContentFieldID'] != "" )
	{
		if ( $content_table->delField( $IN['ContentFieldID'] ) )
		{
			...
			$cache->makeCache( "content_model" );
			cleardir( SYS_PATH."sysdata/cache/", "index.html;.htaccess" );
			_goto( "edittable&TableID={$IN['TableID']}", "table_delfield_ok" );
		}
		else
		{
			_goto( "edittable&TableID={$IN['TableID']}", "table_delfield_fail" );
		}
	}
	break;
case "indexField" :
	if ( $IN['ContentFieldID'] != "" )
	{
		if ( $content_table->indexField( $IN['ContentFieldID'] ) )
		{
			...
			$cache->makeCache( "content_model" );
			cleardir( SYS_PATH."sysdata/cache/", "index.html;.htaccess" );
			_goto( "edittable&TableID={$IN['TableID']}", "table_indexfield_ok" );
		}
		else
		{
			_goto( "edittable&TableID={$IN['TableID']}", "table_indexfield_fail" );
		}
	}
	break;
case "delFieldIndex" :
...
case "orderfield_submit" :
	if ( !empty( $IN['FieldString'] ) )
	{
		...
		if ( $content_table->OrderField( $Fields ) )
		{
			...
			$cache->makeCache( "content_model" );
			cleardir( SYS_PATH."sysdata/cache/", "index.html;.htaccess" );
			_goto( "edittable&TableID={$IN['TableID']}", "table_reOrdefield_ok" );
		}
		_goto( "edittable&TableID={$IN['TableID']}", "table_reOrdefield_fail" );
	}
case "field_listdisplay" :
	if ( $IN['ContentFieldID'] != "" )
	{
		if ( $content_table->EnableFieldListDisplay( $IN['ContentFieldID'], $IN['display'] ) )
		{
			...
			$cache->makeCache( "content_model" );
			cleardir( SYS_PATH."sysdata/cache/", "index.html;.htaccess" );
			_goto( "edittable&TableID={$IN['TableID']}" );
		}
		else
		{
			_goto( "edittable&TableID={$IN['TableID']}" );
