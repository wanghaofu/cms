...
if(empty($IN[NodeID]) && empty($IN[TCID])) {
	...
	if(!empty($IN['targetFile'])) {
		...
		if($psn->fileExists($IN[PATH]."/".$tempFileName)) {
			//Get the editing temporary template file	 
			$content_src = $psn->read((empty($IN[PATH])?'':$IN[PATH]."/"), $tempFileName);
			$psn->close();
			$content = empty($content_src) ? '' : $publish->fetchIndex($IN[NodeID], CACHE_DIR."tmp".$IN[PATH]."/", $tempFileName."v");
...
function make_include_readable($content){
	$pattern = "/<include[\\s]+(.*)[\\s]*[\\/]?>/isU";
	$content = preg_replace_callback($pattern,"make_include_readable_",$content);
	return $content;
}

function make_include_readable_($matches){
	if(preg_match("/file=[\"\\']([^\"\\']*)[\"\\']/siU",$matches[1],$out)){
		return parse_include_format($out[1]);
	}else{
		return "";
	}
}
...
function parse_include_format( $string ){
	...
	if( strpos( "hawking".$string, "file:" ) )
	{
		...
		$psn->connect('file::'.dirname($string));
		$path = "./";
		$targetFile = basename($string);
	}
	...
	if(!isset($return)){
		$return = $psn->read($path,$targetFile);
	}
	return make_include_readable($return);
}
...
if($IN['o'] == 'edit') {
	...
	$psn->connect($psnInfo[PSN]);
	$psn->isLog = false;
	$content_view = make_include_readable($update_content);
	...
	print "成功地update";
	echo "<script>\n
						try{parent.window.opener.refreshWorkArea();}catch(e){}	
						window.close();
						</script>\n";	
