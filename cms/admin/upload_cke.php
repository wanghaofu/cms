...
if ($IN[o] == 'select' && isset($IN[NodeID])) {
	...
	if( !is_array($_FILES['uploadFile']['name']) ){
		$tmp = $_FILES['uploadFile'];
		$_FILES['uploadFile'] = array();
		foreach($tmp as $k => $v){
			$_FILES['uploadFile']["$k"] = array( $v );
		}
	}
	switch($IN[type]) {
		case 'img':
			if ($IN[mode] == 'single') {
				...
				if($IN[makeMini] == '1' && $IN['mkHTML'] != '1') {
					$errmsg.="data.push('".$upload->saveMiniFile[0]."');";
					$errmsg.="data.push('".$upload->saveFile[0]."');";
				} elseif($IN[makeMini] == '1' && $IN['mkHTML'] == '1') {
					$errmsg.="data.push('".$upload->saveMiniFile[0]."');";

					//format : cmsware://publish.link.cw?IndexID=456&NodeID=3
					$errmsg.="data.push('cmsware://publish/url.cw?IndexID=".$returnIndexID."');";
				...
				$errmsg .= "var message = '".$show_message."';";
				$errmsg .= "var call_back=function(){
					alert(message);
					this.getDialog().getContentElement( 'Link', 'txtUrl' ).setValue(data[1]);
					return true;
				};";
				$errmsg .= "window.parent.CKEDITOR.tools.callFunction( " . $IN["CKEditorFuncNum"] . ", data[0] , call_back );";

				$TPL->assign("msg", $errmsg);
				$TPL->display("upload_cke_complete.html");
		...
		//['tmp_name'][$key]
			...
			$errmsg.= "var data = [];";
			$errmsg .= "data.push('".$upload->saveFile[0]."');";
			$errmsg .= "data.push('".$src_name."');";
			$errmsg .= "var message = '".$show_message."';";
			$errmsg .= "var call_back=function(){
				alert(message);
				if(data[0]!=''){
					this.getDialog().getContentElement( 'info', 'filename' ).setValue(data[1]);
				}
				return true;
			};";
			$errmsg .= "window.parent.CKEDITOR.tools.callFunction( " . $IN["CKEditorFuncNum"] . ", data[0] , call_back );";
		...
		case 'flash':
			...
			$errmsg = "var message = '" . $show_message . "',"
			. "data = '" . $upload->saveFile[0] . "';";
			$errmsg .= "window.parent.CKEDITOR.tools.callFunction( " . $IN["CKEditorFuncNum"] . ", data , message );";
			$TPL->assign("msg", $errmsg);
			$TPL->display("upload_cke_complete.html");
			exit;
			break;
