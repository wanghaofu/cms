...
switch($IN[o]) {
	...
	case 'edit':
		...
		if($psn->fileExists($tempFileName)) {
			...
			$psn->connect($psnInfo[PSN]);
			$psn->isLog = false;
			$content_view = make_include_readable($content);
	...
	case 'edit_submit':
		...
		//	$psn->delFile($path, $tempFileName);
			...
			$psn->isLog = false;
			$content_view = make_include_readable($IN[content]);			
	...
	case 'updateTmpSubmit':
		...
		$psnInfo[PSN] = 'file::'.CACHE_DIR."tmp";
		$psnInfo[PSN] .= strpos($referer,'&o=add&') ? $IN[PATH] : "";
		$psn->connect($psnInfo[PSN]);
		$psn->isLog = false;
		$content_view = make_include_readable($IN[updateContent]);
		$psn->put("/".$tempFileName, $IN[updateContent]);
		$psn->put("/".$tempFileName."v", $content_view);
		echo "<script type='text/javascript'>" .
					//"alert('$referer');" .
					"parent.window.document.getElementById('BlockMode').src=parent.window.document.getElementById('BlockMode').src;".
					"</script>";die;
