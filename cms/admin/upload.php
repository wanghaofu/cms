...
else if ( $IN[o] == "display" && isset( $IN[NodeID] ) )
{
	$TPL->assign( "NodeID", $IN[NodeID] );
	switch ( $IN[type] )
	{
	case "img" :
		if ( $IN[mode] == "single" )
		{
			$TPL->display( "upload_img_single.html" );
		}
		else if ( $IN[mode] == "multi" )
		{
			$TPL->display( "upload_img_multi.html" );
		}
		else if ( $IN[mode] == "one" )
		{
			$TPL->display( "upload_img_one.html" );
		}
		else
		{
			$TPL->display( "upload_img_single.html" );
...
else if ( $IN[o] == "upload" && isset( $_FILES['uploadFile'] ) && isset( $IN[NodeID] ) )
{
	switch ( $IN[type] )
	{
	case "img" :
		if ( $IN[mode] == "single" )
		{
			...
			$errmsg .= "<script>parent.add('".$upload->saveFile[0]."',parent.document.form.mdoc);"."alert('".$show_message."');"."parent.document.all.preview_img.src='".$upload->saveFile[0]."';"."parent.document.all.preview_img.style.display='';"."</script>";
		...
		else if ( $IN[mode] == "multi" )
		{
			...
			$errmsg .= "<script>parent.document.form.img_url.value='".$img_url."';"."parent.document.form.img_url_s.value='".$img_url_s."';"."parent.document.all.submit_change.innerHTML='<input type=button  name=yes value=\\'".$_LANG_ADMIN['insert_img']."\\' onclick=\"return insertImg();\"> ';"."alert('".$show_message."');"."</script>";
			$TPL->assign( "msg", $errmsg );
			$TPL->display( "upload_complete.html" );
			exit( );
		}
		else if ( $IN[mode] == "one" )
		{
			...
			else if ( $IN[makeMini] == "1" && $IN['mkHTML'] == "1" )
			{
				$errmsg .= "parent.img_url.value='".$upload->saveMiniFile[0]."';";
		...
		else if ( $IN[mode] == "picker" )
		{
			...
			$errmsg .= "<script>alert('".$show_message."');"."parent.returnImg('".$upload->saveFile[0]."');"."</script>";
			$TPL->assign( "msg", $errmsg );
			$TPL->display( "upload_complete.html" );
			exit( );
		}
	...
	case "attach" :
		...
		$errmsg .= "<script>alert('".$show_message."');"."parent.returnInfo('".$upload->saveFile[0]."', '".$src_name."','".$suffix."','".$publish_url."');"."</script>";
		$TPL->assign( "msg", $errmsg );
		$TPL->display( "upload_complete.html" );
		exit( );
		break;
	case "flash" :
		...
		$errmsg .= "<script>alert('".$show_message."');"."parent.returnInfo('".$upload->saveFile[0]."');"."</script>";
		$TPL->assign( "msg", $errmsg );
		$TPL->display( "upload_complete.html" );
		exit( );
		break;
