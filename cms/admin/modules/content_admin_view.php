...
{	 
	...
	if(src.innerHTML == '[<?=$_LANG_SKIN['preview']?>]') {
		...
		targetCode.value = targetPreview.innerHTML;
		src.innerHTML = '[<?=$_LANG_SKIN['preview']?>]';
...
foreach( $tableInfo as $key=>$var) {
	...
	if(!empty($pInfo[$var['FieldName']])) {
		if(!empty($SYS_ENV['ContentViewMode'])) {
