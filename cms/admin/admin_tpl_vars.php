...
case "add_submit" :
	...
	if ( !empty( $IN[NodeIDs] ) )
	{
		...
		{
			if ( !empty( $IN[SubNodeIDs] ) && in_array( $var, $IN[SubNodeIDs] ) )
			{
				continue;
			}
			else
	...
	if ( $tpl_vars->add( ) )
	{
		...
		_goto( "view", "add_tpl_vars_ok" );
	}
	else
	{
		_goto( "view", "add_tpl_vars_fail" );
	}
	break;
case "edit" :
	if ( !empty( $IN[Id] ) )
...
case "edit_submit" :
	...
	{
		cleardir( SYS_PATH."sysdata/cache/", "index.html;.htaccess" );
		_goto( "view", "edit_tpl_vars_ok" );
	}
	else
	{
		_goto( "view", "edit_tpl_vars_fail" );
	}
	break;
case "del" :
	if ( empty( $IN[Id] ) )
	...
	{
		cleardir( SYS_PATH."sysdata/cache/", "index.html;.htaccess" );
		_goto( "view", "del_tpl_vars_ok" );
	}
	else
	{
		_goto( "view", "del_tpl_vars_fail" );
	}
	break;
}
