...
unset( $License );
$SYS_ENV['CMSware_Mark'] = str_replace( "{date}", date( "Y-m-d H:i:s", time( ) ), "<!-- Published at {date}, Powered by {version} -->" );
...
define( "ADD_META_MARK", 1 );
$SYS_ENV['CMSware_Powered'] = "- Powered by CMSware";
...
{
	$SYS_ENV['DialogFitXP'] = 1;
}
if ( !empty( $IN['message'] ) )
{
	$TPL->assign( "message", $_LANG_ADMIN[$IN['message']] );
}
if ( !empty( $IN['error_message'] ) )
{
	$TPL->assign( "error_message", $_LANG_ADMIN[$IN['error_message']] );
}
if ( $SYS_ENV['isLogAdmin'] == 1 )
{
	$logAdmin = new logAdmin( );
	$logAdmin->addAdminLog( $sys->session['sUserName'], $IN['IP_ADDRESS'], $IN );
}
?>
