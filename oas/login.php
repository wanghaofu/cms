	...
	default:
		if($Auth->isLogin())//已经登陆
		{
			...
			if(strtolower($SYS_ENV['main_domain'])=='localhost' or empty($SYS_ENV['main_domain'])) {
				...
				setcookie('OAS_COOKIE',$OAS_COOKIE,$SYS_ENV['cook	ie_timeout'],'/',$SYS_ENV['main_domain']);
			}
			de($_COOKIE['OAS_COOKIE']);
			header("Location: {$forward}");
		} else   //没有登陆
		{
			if(!isset($IN['formcwps'])) {
				$referer = $SYS_ENV['OAS_URL']."login.php?formcwps=true&forward=".rawurlencode($forward);
				$Auth->login($referer);  //转到cwps 把此处改为直接传输登陆信息过去
			} else {
				header("Location:{$forward}");
			}
		}
