...
if($TPL->caching and $TPL->is_cached($tplname, $cacheId)) {
	...
	//Add by easyT, 2007.1.9, 为动态发布内容页入口增加列表分页功能，按index.php首页入口格式给content.php增加系统$Page变量，在动态发布的内容页里就可以用CMS_LIST的分页功能了，同样用IndexPage()来获取分页链接, 在IndexPage()里处理替换内容页入口content.php需要的参数IndexID
	$SYS_ENV[tpl_pagelist][page] = $Page;
	$SYS_ENV[tpl_pagelist][filename] = $NodeInfo['ContentPortalURL'];
	//Add by easyT, 2007.1.9, end

	$publishInfo['NodeInfo']['ResourcePSN']= $NodeInfo['ResourcePSN'];
	$publishInfo['NodeInfo']['ResourceURL']= $NodeInfo['ResourceURL'];
