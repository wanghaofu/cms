...
switch ($IN['o']) {
	...
	case 'post' :
		...
		{
			session_start();
			if(empty($_SESSION['sessionValid']) || empty($IN['validCode']))
			{ //如果没有通过validCode.php注册$_SESSION['sessionValid']
		...
		if ($IN['content'] != '') {
			...
			if ( preg_match('/([a-z]{5})/i',$IN['content'],$matches ) ) {
				$params=array(0=>$matches[1]);
				goback("comment.badwords",$params);
			}
